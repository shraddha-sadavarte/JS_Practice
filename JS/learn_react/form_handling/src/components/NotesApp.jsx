...
const NotesApp = () => {
  ...
  // When page reload  load notes from locaStorage
  useEffect(() => {
    if (hasLoaded.current) return;  // prevent second run

    const storedNotes=JSON.parse(localStorage.getItem("task") || "[]");
    if (storedNotes){
      setTask(storedNotes)
    }
      ...
      <div className='lg:w-1/2 lg:border-l-2  p-10'>
        ...
        <div className='flex flex-wrap items-start justify-start gap-5 mt-6 h-[90%] overflow-auto'>
          {task.map(function (elem, idx) {
            ...
            return <div key={idx} className=" flex justify-between flex-col items-start relative h-52 w-40 rounded-xl text-black pt-9 pb-4 px-4" 
            style={{ 
